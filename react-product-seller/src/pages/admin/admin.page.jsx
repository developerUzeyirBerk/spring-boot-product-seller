...
const AdminPage = () => {
    ...
    const [selectProduct, setSelectProduct] = useState(new Product('','',0));
    ...
    const deleteProduct = () => {
      ProductService.deleteProduct(selectProduct).then(_ => {
            setProductList(productList.filter(x=> x.id !== selectProduct.id));
      }).catch(err => {
    ...
    return(
        <div>
            ...
            <ProductSave ref={saveComponent} product={selectProduct} onSaved={(p) => saveProductWatcher(p)} />
            <ProductDelete ref={deleteComponent} onConfirmed={() =>deleteProduct()}/>

            <p>Developed by Berk ALTINOK</p>
        </div>
    );
};
